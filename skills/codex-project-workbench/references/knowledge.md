...
检索默认使用 `strategy:"bm25"`。`search --strategy smart` 和新任务的 `knowledge.strategy:"smart"` 提供显式对照入口：同一次来源核对后做词项变体查询、排名融合及中文词法去重重排。它不调用模型，不具备向量语义或自动判断旧结论失效的能力。仅在对应项目评测支持且本次任务明确选择时使用；普通开发不同时跑两条路线，不因候选实现已通过代码测试就切换默认策略。省略 strategy 可保持原行为，已冻结任务包不会被新策略改写。
