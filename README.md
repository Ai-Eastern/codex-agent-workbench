...
| 最近一次框架测试 | 145 项：144 通过、0 失败、1 项既有平台跳过 | 本地自动化测试，不替代 GUI 或用户验收 |
...
执行者完成必要自测后，按新任务包内的 resultTool 调用 `submit`，自动生成结果与哈希；PM 再用 continue 完成正式验收。长输出可加 `--view compact`，完整详情自动保存在 controlRoot/views；收到 `needsRead:true` 时先读详情，不能重新执行原派工取全文。较短的状态与完成回执保持默认即可。
...
下一项有界改进优先提高非法候选字段报错的可操作性，减少为修正输入而读取源码；复用已有检查和引用证据，避免加长 Skill 或再增加固定审核轮。整轮收益仍以真实交付为准。
