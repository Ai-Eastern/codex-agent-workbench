...
| 最新恋语真实交付 | 训练包只读验真 CLI，17 个用例、一次正式集成验收、1 条知识保存；采用 direct | 离线命令证据，不代表训练、人工或生产验收 |
| 最近一次框架测试 | 127 项：126 通过、0 失败、1 项既有平台跳过 | 本地自动化测试，不替代 GUI 或用户验收 |

最新恋语样本的 PM 完整轮次约 **7分31秒**，控制器记录内执行与等待约 **229.31 秒**，正式验收命令约 **1.18 秒**。该样本的验收命令耗时很小，执行与等待仍不能全部拆分为纯编码、工具或空等。

本轮还观察到：原生派送应答返回后，GM 额外保存一份重复派工报告，间隔约 **34.18 秒**。已更新规则并验证可从原回执提取所需数据；这个区间与 PM 执行重叠，**不能当作已节省的端到端时间**，修复后的真实派工收益仍待观察。

| 最新样本成本 | 原始总 Token（含缓存） | 非缓存输入 | 输出 |
|---|---:|---:|---:|
| 产品＋管理 | 2,893,466 | 263,887 | 19,147 |
| 一次性框架建设，截点下界 | 4,923,069 | 114,024 | 28,501 |

成本截点为 2026-09-12 13:01:08 UTC，维护者尚未结束，其后报告和发布消耗未计入。缓存输入已包含在输入中，不重复累加，不换算 Codex 订阅费用。前后真实任务不同；本轮产品耗时和原始 Token 没有比上一项更低，尚未证明建设回本或整体提效。

证据：[最新交付、计时与修复](docs/task-timing-results-20260912.md) · [阶段交接前后观察](docs/stage-handoff-results-20260912.md) · [多项目验证](docs/dispatch-scale-results-20260912.md)。
...
沿下一项正常开发验证现有流程：总经理直接派工，PM 按依赖组织工作，完成一次验收与知识保存；观察重复派工报告、重复验收和完成后反复确认是否消失。
...
| 真实开发与成本 | [首次成本分账](docs/ruflo-real-task-cost-20260912.md) · [阶段交接](docs/stage-handoff-results-20260912.md) · [多项目验证](docs/dispatch-scale-results-20260912.md) |
